...
vi.mock('@/context/theme-provider', () => ({
  useTheme: () => ({
    ...
    availableColorThemes: [
      { id: 'default', name: 'Slate', description: '' },
      { id: 'ember', name: 'Ember', description: '' },
    ],
  }),
}))
...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('renders the command palette when the palette is open', async () => {
    ...
    await expect
      .element(getByText(en.nav.link.lowCarbonCockpit))
      .toBeInTheDocument()
    await expect
      .element(getByText(en.nav.link.alarmManagement))
      .toBeInTheDocument()
  })
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText(en.nav.link.userManagement))
