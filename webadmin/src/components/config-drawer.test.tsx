...
async function openDrawer(screen: RenderResult) {
  await userEvent.click(
    screen.getByRole('button', {
      name: /^(Open theme settings|打开主题设置)$/i,
    })
  )
  await expect
    .element(screen.getByText(/^(Theme Settings|主题设置)$/i))
    .toBeInTheDocument()
}
...
describe('ConfigDrawer (integration)', () => {
  ...
  it('opens the drawer and renders the sections', async () => {
    ...
    await expect
      .element(drawer.getByText(/^Color palette$/i))
      .toBeInTheDocument()
  ...
  it('reset restores defaults across sidebar/theme/layout/direction', async () => {
    ...
    await vi.waitFor(() =>
      expect(getCookie('vite-ui-color-theme')).toBeUndefined()
    )
    await vi.waitFor(() => expect(getCookie('layout_variant')).toBe('inset'))
    await vi.waitFor(() => expect(getCookie('layout_collapsible')).toBe('icon'))
    await vi.waitFor(() =>
  ...
  it('global reset clears interface language cookie', async () => {
    setCookie('vite-ui-locale', 'zh')
    const screen = await renderConfigDrawer()
    ...
    await userEvent.click(
      screen.getByRole('button', {
        name: /reset all settings to default values|将所有设置恢复为默认值/i,
      })
    )

    await vi.waitFor(() => expect(getCookie('vite-ui-locale')).toBeUndefined())
  })

  describe('color palette preference', () => {
    it('applies ember palette to html and cookie', async () => {
      const screen = await renderConfigDrawer()
      await openDrawer(screen)
      await userEvent.click(
        screen.getByRole('radio', { name: /select ember/i })
      )
      await vi.waitFor(() =>
        expect(document.documentElement.getAttribute('data-color-theme')).toBe(
          'ember'
      ...
      expect(getCookie('vite-ui-color-theme')).toBe('ember')
    })

    it('resets color palette via section control', async () => {
      setCookie('vite-ui-color-theme', 'ember')
      const screen = await renderConfigDrawer()
      await openDrawer(screen)
      await vi.waitFor(() =>
        expect(
      ...
      await userEvent.click(
        screen.getByRole('button', {
          name: /reset color palette to default/i,
        })
      )
      await vi.waitFor(() =>
        expect(
